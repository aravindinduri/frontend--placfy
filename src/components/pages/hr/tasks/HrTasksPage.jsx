...
import {
    CheckSquare, Search, Filter, Clock,
    AlertCircle, CheckCircle, Calendar,
    User, BarChart3, Loader2 as LucideLoader
} from 'lucide-react';
import { getStoredToken, getSessionToken } from "../../../utils/authToken";
import HrTaskTrackingPage from './HrTaskTrackingPage';
...
const HrTasksPage = () => {
    ...
    const fetchWorkspaceAndTasks = async () => {
        try {
            ...
            // 2. Fetch MY Tasks (only assigned to me)
            let url = `http://localhost:8000/api/v1/workspaces/${slug}/my-tasks/`;
            if (filterStatus !== 'all') {
    ...
    return (
        <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex items-center justify-between py-6 shrink-0">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 tracking-tight uppercase">My Tasks</h1>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Tasks assigned to you</p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pr-2 pb-6 scrollbar-hide">
                    {tasks.map(task => (
                        <div key={task.id} className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100 hover:scale-[1.02] transition-all group flex flex-col relative cursor-pointer" onClick={() => openTracking(task)}>
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <button
                                    onClick={(e) => { e.stopPropagation(); openTracking(task); }}
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-slate-100 border-2 border-white shadow-sm flex items-center justify-center text-slate-400 font-bold text-xs overflow-hidden">
                                        {task.created_by_name ? (
                                            <img
                                                src={`https://api.dicebear.com/7.x/initials/svg?seed=${task.created_by_name}`}
                                                alt="avatar"
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Assigned By</span>
